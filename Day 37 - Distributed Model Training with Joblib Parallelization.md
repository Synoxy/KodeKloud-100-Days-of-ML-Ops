...
🛠️ **Solution:**
1. Update ```featurize.py``` as mentioned below:
```text
input_path = config["data"]["processed_path"]
```
2. Run ```python3 run_pipeline.py``` and verify the below:
   -  evaluation.json, the report carries accuracy, f1, and roc_auc as numeric values.
   -  row count of data/features/features.csv equals the row count of data/processed/train_clean.csv and is strictly less than the 200-row.
   -  Exactly one MLflow run exists in the training-pipeline experiment, carrying params.model_type, params.n_estimators, params.max_depth, and the three evaluation metrics.